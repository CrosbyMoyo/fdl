...
spark.sql(f'''
    CREATE OR REPLACE TABLE {env_vars.gold_catalog}.fin_general_ledger.dim_consolidation_reporting_item_hierarchy
    (
        consolidation_reporting_item_skey BIGINT
            COMMENT 'Consolidation Reporting Item Skey',
        ...
        reporting_item_hierarchy_node STRING
            COMMENT "The hierarchy node ID. This is the node ID at the root of the hierarchy",
        

        -- Payload
        reporting_item_description STRING
            COMMENT "The description of the consolidation reporting item",
    ...
    CONSTRAINT pk_dim_consolidation_reporting_item PRIMARY KEY (consolidation_reporting_item_skey)
        )
        CLUSTER BY 
            AUTO;
''')