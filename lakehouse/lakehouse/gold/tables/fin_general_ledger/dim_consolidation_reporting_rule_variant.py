...
spark.sql(f'''
    CREATE OR REPLACE TABLE {env_vars.gold_catalog}.fin_general_ledger.dim_consolidation_reporting_rule_variant (
        --Key
        consolidation_reporting_rule_variant_skey BIGINT
            COMMENT 'Consolidation Unit Skey',
        reporting_rule_variant STRING
            COMMENT 'Reporting Rule Variant',
        reporting_item_hierarchy STRING
            COMMENT 'Reporting Item Hierarchy',
        
        -- Payload
        consolidation_chart_of_accounts STRING
            COMMENT 'Consolidation Chart of Accounts',
        description STRING
            COMMENT 'Reporting Rule Variant Description',
    ...
    CONSTRAINT pk_dim_consolidation_reporing_rule_variant PRIMARY KEY (consolidation_reporting_rule_variant_skey)
    )
    CLUSTER BY
        AUTO;
''')