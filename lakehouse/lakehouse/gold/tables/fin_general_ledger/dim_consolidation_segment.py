...
spark.sql(f'''
    CREATE OR REPLACE TABLE {env_vars.gold_catalog}.fin_general_ledger.dim_consolidation_segment (
        --key
        consolidation_segment_skey BIGINT
            COMMENT 'Consolidation Segment Skey',
        segment STRING
            COMMENT 'Segment',
    ...
    CONSTRAINT pk_dim_consolidation_segment PRIMARY KEY (consolidation_segment_skey)
    )
    CLUSTER BY
        AUTO;
''')