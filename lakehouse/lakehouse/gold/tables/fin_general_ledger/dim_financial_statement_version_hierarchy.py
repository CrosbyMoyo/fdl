...
spark.sql(f'''
    CREATE OR REPLACE TABLE {env_vars.gold_catalog}.fin_general_ledger.dim_financial_statement_version_hierarchy
    (
        -- Keys
        financial_statement_version_skey BIGINT
            COMMENT "The SKEY for Financial Statment version", 
        hierarchy_id STRING
    ...
    CONSTRAINT pk_dim_financial_statement_version PRIMARY KEY (financial_statement_version_skey)
        )
        CLUSTER BY
            AUTO;
''')