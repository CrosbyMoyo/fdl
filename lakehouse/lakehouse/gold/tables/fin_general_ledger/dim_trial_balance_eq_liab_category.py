...
spark.sql(f'''
    CREATE OR REPLACE TABLE {env_vars.gold_catalog}.fin_general_ledger.dim_trial_balance_eq_liab_category (
        -- Keys
        trial_balance_eq_liab_category_skey BIGINT
            COMMENT 'Unique key for the record',
        rank INT
            COMMENT 'Rank',
        parent_id STRING
            COMMENT 'Parent ID',
        tb_category_node STRING
            COMMENT 'Trial Balance Eq Liab Category Hierarchy Node',

        -- Payload
        tb_category STRING
            COMMENT 'Trial Balance Eq Liab Category',
