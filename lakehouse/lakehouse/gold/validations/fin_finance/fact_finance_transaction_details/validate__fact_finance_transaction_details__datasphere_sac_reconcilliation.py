...
# MAGIC - 82 records in datasphere for SAC that are not appearing in vivid (null in the below query) see [bug #1261](https://dev.azure.com/VivoEnergy/Azure%20SAP%20Data%20Reporting/_sprints/taskboard/Vivid%20Platform%20Team/Azure%20SAP%20Data%20Reporting/Sprint%208?System.AssignedTo=%40me&workitem=1261)
# MAGIC - 191 records with differing volume litres see [bug #1259](https://dev.azure.com/VivoEnergy/Azure%20SAP%20Data%20Reporting/_workitems/edit/1259)
# MAGIC - 201 V.001 records with differing vcode amounts see [bug #1262](https://dev.azure.com/VivoEnergy/Azure%20SAP%20Data%20Reporting/_sprints/taskboard/Vivid%20Platform%20Team/Azure%20SAP%20Data%20Reporting/Sprint%208?System.AssignedTo=%40me&workitem=1262)
# MAGIC - Known difference of 189 records for Volume in 2024-12-01 which exist in Datasphere but don't have a source. Confirmed in bug #1198 that these are not actually a reconciliation issue.
# MAGIC
# MAGIC
# MAGIC
...
validation = spark.sql(
  f'''
    ...
    select 
      ...
      , gl.balance_sheet_account_flag
      , CAST(substring(fact.actual_plan_code, 6, 4) AS INT) || '|' || profit_center || '|' || company_code || '|' || gl_account || '|' || vcode || '|' || date as key
      ...
      where 1=1
        and datasource.datasource_name = 'SAC'
    )
...
spark.sql(
    f'''
    select 
      CAST(substring(fact.actual_plan_code, 6, 4) AS INT) AS planyear
      , ifnull(nullif(comp.company_code, ''), '¬¬') as company_code
      , ifnull(nullif(gl.gl_account, ''), '¬¬') as gl_account
      , ifnull(nullif(vcode.vcode, ''), '¬¬') as vcode
      , ifnull(nullif(pc.profit_center, ''), '¬¬') as profit_center
      ...
      , CASE WHEN left(fact.actual_plan_code, 4) = 'PLAN' then ifnull((left(date_key, 8) || '01')::date, '1900-01-01'::date) else ifnull(date_key, '1900-01-01'::date) end as date
      , gl.balance_sheet_account_flag
      , CAST(substring(fact.actual_plan_code, 6, 4) AS INT) || '|' || profit_center || '|' || company_code || '|' || gl_account || '|' || vcode || '|' || source || '|' || date as key
      , amount_local_currency as vvd_amount_lc
      , amount_group_currency as vvd_amount_gc
      , amount_group_currency_plan_rate as vvd_amount_gc_plan
      , amount_group_currency_month_end as vvd_amount_gc_monthly_end
      ...
      , vcode_amount_local as vvd_vcode_amount_local
      , vcode_amount_group as vvd_vcode_amount_group
      from vivid_dev_gld.fin_finance.fact_finance_transaction_details fact
        left join  vivid_dev_gld.fin_general_ledger.dim_gl_account gl
        on fact.gl_account_skey = gl.gl_account_skey
        left join  vivid_dev_gld.ca_cross_application_components.dim_company_code comp
        on fact.company_code_skey = comp.company_code_skey
        left join  vivid_dev_gld.fin_controlling.dim_profit_center pc 
          on fact.profit_center_skey = pc.profit_center_skey
        left join  vivid_dev_gld.fin_general_ledger.dim_vcodes vcode
        on fact.vcode_skey = vcode.vcode_skey
        left join  vivid_dev_gld.ca_cross_application_components.dim_datasource datasource
        on fact.datasource_skey = datasource.datasource_skey
      where 1=1
        and datasource.datasource_name = 'SAC'
    '''
