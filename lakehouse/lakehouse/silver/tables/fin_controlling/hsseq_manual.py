...
spark.sql(f'''
    CREATE OR REPLACE TABLE {env_vars.silver_catalog}.fin_controlling.hsseq_manual (
        -- KEY
        year_month STRING
            COMMENT 'date key',
        actual_target STRING
            COMMENT 'represents the actual and target values',

        -- PAYLOAD
        fatal_incidents DOUBLE
            COMMENT 'fatal incidents', 
        ...
        spills DOUBLE
            COMMENT 'number of spills', 
        recordable_case_frequency_cont DOUBLE
            COMMENT 'total recordable case frequency cont',
        recordable_case_frequency_emp DOUBLE
            COMMENT 'total recordable case frequency emp',
