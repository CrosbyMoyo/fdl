...
spark.sql(f'''
    CREATE OR REPLACE TABLE {env_vars.silver_catalog}.fin_controlling.special_items (
        -- KEY
        year_month STRING
            COMMENT 'special items date',
        special_item_code STRING
            COMMENT 'special items values',

        -- PAYLOAD
        amount_mtd DECIMAL(8,2)
            COMMENT 'amount month to date', 
        amount_ytd DECIMAL(8,2)
            COMMENT 'amount year to date',
