...
spark.sql(f'''
    CREATE OR REPLACE TABLE {env_vars.silver_catalog}.fin_finance.oil_prices (
        -- KEY
        date_key DATE
            COMMENT 'Date key',
        ...
        symbol STRING
            COMMENT 'Symbol',
        trading_unit STRING
            COMMENT 'Trading unit',
        currency STRING
            COMMENT 'Currency',

        -- PAYLOAD
        price_local_currency DECIMAL(38,2)
            COMMENT 'Price local currency', 
        price_group_currency DECIMAL(38,2)
            COMMENT 'Price group currency', 
