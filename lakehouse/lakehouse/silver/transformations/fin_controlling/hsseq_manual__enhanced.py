...
metadata_filename = "silver.hsseq.yaml"
...
spark.sql(
    f'''
        SELECT
            concat(substring(hs.year_month, 1, 4), '-', substring(hs.year_month, 5, 2)) AS year_month,
            upper(regexp_replace(hs.actual_target,'public.' , '' )) AS actual_target,
            hs.fatal_incidents,
            hs.life_saving_rule_violations,
            hs.lost_time_injury_frequency,
            hs.potential_incidents,
            hs.severe_motor_vehicle_incident_rate,
            hs.spills_100kg,
            hs.spills,
            hs.recordable_case_frequency_cont,
            hs.recordable_case_frequency_emp,
            hs.__etl_keys_fprint,
            hs.__etl_effective_from,
            hs.__etl_effective_to,
            hs.__etl_is_active,
            hs.__etl_is_deleted
        FROM 
            {source_tablename} AS hs
    '''
