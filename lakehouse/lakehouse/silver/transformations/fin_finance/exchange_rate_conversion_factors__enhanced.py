...
metadata_filename = "silver.tcurf.yaml"
...
spark.sql(
    f'''
        SELECT 
            src.* EXCEPT(src.ratio_from, src.ratio_to)
            ,cast(round(src.ratio_from, 0) AS INTEGER) AS ratio_from
            ,cast(round(src.ratio_to, 0) AS INTEGER) AS ratio_to
        FROM
