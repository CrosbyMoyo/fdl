...
metadata_filename = "silver.cash_flow_subcategory.yaml"
...
reporting_item_hierarchy_tablename = f'{env_vars.silver_catalog}.{metadata.sources_2partname("consolidation_reporting_item_hierarchy", include_schemaversion=True)}'

cash_flow_category_tablename = f'{env_vars.silver_catalog}.{metadata.sources_2partname("cash_flow_category", include_schemaversion=True)}'
...
enhanced = spark.sql(f'''
    SELECT
        ...
        c.cf_category AS category_description,
        c.rank AS category_rank
    FROM
        base AS b
        JOIN {cash_flow_category_tablename} AS c
