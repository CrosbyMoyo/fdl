...
# MAGIC %md
# MAGIC ## 2vr_fi_facccmapping_001 Bronze to Silver
# MAGIC
# MAGIC Generic Notebook moves the `{bronze}.fivetran_s4p.2vr_fi_facccmapping_001` data to a staging table defined in the metadata.
# MAGIC
# MAGIC This notebook further refines that data, and adds the payload fingerprint.  Then merges the data into `{silver}.fin_controlling.faccc_mapping` table.
...
metadata_filename = "silver.2vr_fi_facccmapping_001.yaml"
...
sci = spark.sql(f'''
    SELECT 
        src.functional_area,
        src.cost_center_functional_area_category,
