...
# MAGIC %md
# MAGIC ## fagl_011qt Bronze to Silver
# MAGIC
# MAGIC Generic Notebook moves the `{bronze}.fivetran_s4p.fagl_011qt` data to a staging table defined in the metadata.
# MAGIC
# MAGIC This notebook further refines that data, and adds the payload fingerprint.  Then merges the data into `{silver}.fin_general_ledger.financial_statement_text_for_items` table.
...
metadata_filename = "silver.fagl_011qt.yaml"
...
am_enhanced = spark.sql(f'''
    SELECT DISTINCT
        ...
        --payload
        source.financial_statement_item_description,
