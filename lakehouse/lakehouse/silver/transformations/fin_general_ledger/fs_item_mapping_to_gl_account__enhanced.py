...
# MAGIC %md
# MAGIC ## FINCS_FSIMAPITM Bronze to Silver
# MAGIC
# MAGIC Generic Notebook moves the `{bronze}.fivetran_s4p.fincs_fsiMAPITM` data to a staging table defined in the metadata.
# MAGIC
# MAGIC This notebook further refines that data, and adds the payload fingerprint.  Then merges the data into `{silver}.fin_controlling.fs_item_mapping_to_gl_account` table.
...
metadata_filename = "silver.fincs_fsimapitm.yaml"
...
facs = spark.sql(f'''
    SELECT 
        ...
        src.account_number,
        src.fs_item,
        first_value(src.account_number) 
            OVER(
                PARTITION BY 
                    src.consolidation_coa, 
                    src.chart_of_accounts, 
...
merge_result = spark.sql(f'''    
    ...
    WHEN MATCHED THEN
        UPDATE SET 
            {match_cols_ddl},
            tgt.first_gl_account = src.first_gl_account,
