...
# MAGIC %md
# MAGIC ## FINCS_FSIMAPASSG Bronze to Silver
# MAGIC
# MAGIC Generic Notebook moves the `{bronze}.fivetran_s4p.fincs_fsimapassg` data to a staging table defined in the metadata.
# MAGIC
# MAGIC This notebook further refines that data, and adds the payload fingerprint.  Then merges the data into `{silver}.fin_controlling.fs_item_mapping_to_special_mapping_version` table.
...
metadata_filename = "silver.fincs_fsimapassg.yaml"
...
facs = spark.sql(f'''
    SELECT 
        src.client,
        src.fs_item_mapping_version,
        src.consolidation_coa,
        src.chart_of_accounts,
        src.valid_to,
        src.valid_from,
        src.valid_to_fiscal_yearperiod,
        src.valid_from_fiscal_yearperiod,
        src.fs_item_mapping_id,
        src.fs_item_mapping_revision,
